...
export class Renderer {
   ...
   async Initialize() {
    ...
    async createAssets() {
        ...
        this.sphereIndexBuffer = this.device.createBuffer(sphereIndexBufferDescriptor);
    ...
    prepareScene(){
        ...
        this.device.queue.writeBuffer(
            this.sphereBuffer,0,
            sphereData, 0, 8 * this.scene.spheres.length
        );

        const nodeData: Float32Array = new Float32Array(8 * this.scene.nodesUsed);
        for(var i = 0; i < this.scene.nodesUsed; i++){
        ...
        this.device.queue.writeBuffer(
            this.nodeBuffer,0,
            nodeData, 0, 8 * this.scene.nodesUsed
        );

        const sphereIndexData: Float32Array = new Float32Array(this.scene.sphereCount);
        for(var i = 0; i < this.scene.sphereCount; i++){
            sphereIndexData[i] = this.scene.sphereIndices[i];
        }

        this.device.queue.writeBuffer(
            this.sphereIndexBuffer,0,
            sphereIndexData, 0, this.scene.sphereCount
        );
