        ...
        if(sphereCount == 0){
            ...
            for(var i:u32 = 0; i < sphereCount; i++){
                ...
                var newRenderState: RenderState = hit_sphere(
                    ray, 
                    objects.sphere[u32(sphereLookup.sphereIndices[i + contents])], 
                    0.001, nearesHit, renderState
                );

                if(newRenderState.hit){
                    nearesHit = newRenderState.t;
...
fn hit_sphere(ray: Ray, sphere: Sphere, tMin: f32, tMax: f32, oldRendeState: RenderState) -> RenderState{
    ...
    var renderState: RenderState;
    renderState.color = oldRendeState.color;
...
fn hit_aabb(ray : Ray, node: Node) -> f32{
    ...
    var t1: vec3<f32> = (node.minCorner - ray.origin) / ray.direction;
    var t2: vec3<f32> = (node.minCorner - ray.origin) / ray.direction;
